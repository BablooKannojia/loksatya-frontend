...
export default function CategoryPage() {
  ...
  // URL Slug से Category Name Decode करें
  const rawCategoryName = params?.slug ? decodeURIComponent(params.slug) : "";
    ...
    async (categoryName, page) => {
      ...
      try {
        ...
        const response = await axios.get(
          `https://admin.loksatya.com/api/article?${queryParams.toString()}`
        );
  ...
  // Handle URL change or Page Change
  useEffect(() => {
    if (rawCategoryName) {
  ...
  // HTML Tags हटाने और Description को ट्रिम करने का फ़ंक्शन
    ...
    <div className="w-full bg-gray-50/50 py-4 font-devanagari">
      ...
      <div className="max-w-[1300px] mx-auto px-3 sm:px-4 flex flex-col lg:flex-row gap-6 items-start">
        ...
        <div className="w-full lg:w-[70%] bg-white rounded-2xl p-4 sm:p-6 border border-gray-100 shadow-xs">
          ...
          {/* Section Header */}
          <div className="border-b-2 border-[#D90429] pb-3 mb-5 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="h-6 w-1.5 bg-[#D90429] rounded-full inline-block"></span>
              <h1 className="text-xl sm:text-2xl font-bold text-gray-900 capitalize tracking-tight">
                {rawCategoryName || "कैटेगरी"}
              </h1>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4 min-h-[400px]">
            {loading ? (
              ...
              <div className="text-center py-16 text-gray-500 text-base">
                इस कैटेगरी में कोई खबर उपलब्ध नहीं है।
              </div>
            )}
          </div>
      ...
      <div className="max-w-[1300px] mx-auto px-3 sm:px-4 mt-6">
